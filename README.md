...
| **dsh ≥ 0.1.7** | **≤ 0.1.8** | 能敲，但不涨功德、也不飘 +1 |

**老 dsh 上别只升木鱼到 0.1.5**——先升宿主，再升插件；暂时不升 dsh 就钉住 `dsh-muyu@0.1.4`。

**≤ 0.1.8 配 dsh ≥ 0.1.7 时，敲击不记功德、也不飘 +1。** 0.1.7 起当前会话不再写在列表的 `current` 上，而由主视图持有那一行（`retainedBy.mainView`）。**0.1.9 起按这个持有关系取当前会话**（更老的宿主仍读 `current`）。已存功德不受影响。
...
| 敲木鱼不涨功德、也不飘 +1（dsh ≥ 0.1.7） | 插件是 **≤ 0.1.8**。升级到 **≥ 0.1.9** 并重启 Web |
