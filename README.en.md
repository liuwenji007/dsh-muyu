...
- Use the lock chip to unlock, drag, then lock again; while unlocked, scroll or use −/+ to scale. Settings can hide the chip.
...
| **dsh ≥ 0.1.7** | **0.1.8** | Overlay and settings page never mount; console shows `exclusive store for factory ... cannot declare persistence` |
...
**0.1.8 on dsh ≥ 0.1.7 is broken** (knocking records no merit and floats no +1): from 0.1.7 the host no longer lets an exclusive factory store declare a persist key, so the fish's slot throws an assembly error at render. **0.1.9 registers a shared handle instead**, which mounts on new and old hosts alike and keeps your stored merit and settings.
...
| Knocking records no merit and floats no +1 (dsh ≥ 0.1.7) | The plugin is **0.1.8**. The console shows `exclusive store for factory ... cannot declare persistence`. Upgrade to **≥ 0.1.9** and restart Web |
